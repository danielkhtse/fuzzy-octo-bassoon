...
describe('search', () => {
  ...
  // Group 1: Organization-based tests
  describe('organization-based data inclusion', () => {
    it('should correctly determine additional data inclusion based on organization name', () => {
      // Test Circle organization
      expect(shouldIncludeAdditionalData(MOCK_ORG_CIRCLE)).to.be.true;

      // Test non-Circle organization
      expect(shouldIncludeAdditionalData(MOCK_ORG_PRENETICS)).to.be.false;
    ...
    it('should include additional data for Circle organization', async () => {
      // Override getManyAndCount for this specific test
      ...
      expect(result.meta.total).to.equal(mockResult.length);
    ...
    it('should exclude additional data for non-Circle organization', async () => {
      const mockResult = testData.filter(
        (result) =>
          result.profile?.organisation.organisationId ===
          MOCK_ORG_PRENETICS.organisationId
      );
      ...
      const result = await search(mockManager, MOCK_ORG_PRENETICS, {});

      expect(result.data).to.have.lengthOf(mockResult.length);
      expect(result.meta.total).to.equal(mockResult.length);
      expect(result.data[0]).to.not.have.any.keys(['patientId', 'resultType']);
      expect(result.data[0]).to.have.all.keys([
        'id',
        'patientName',
        'sampleBarcode',
  ...
  // Group 2: Query parameter filters
  ...
  // Group 4: Result mapping tests
  describe('mapResultToResponse', () => {
    it('should map Result to Response with additional data for Circle organization', () => {
      ...
      const response = mapResultToResponse(testResult as Result, true);
    ...
    it('should map Result to Response without additional data for non-Circle organization', () => {
      const testResult: Partial<Result> = {
        resultId: '123e4567-e89b-12d3-a456-426614174005',
        type: ResultType.antigen,
        activateTime: new Date('2024-03-20T10:00:00Z'),
      ...
      const response = mapResultToResponse(testResult as Result, false);
      ...
      expect(response).to.not.have.property('patientId');
      expect(response).to.not.have.property('resultType');
      expect(response.activationDate.toISOString()).to.equal(
        testResult.activateTime?.toISOString()
      );
      expect(response.resultDate.toISOString()).to.equal(
